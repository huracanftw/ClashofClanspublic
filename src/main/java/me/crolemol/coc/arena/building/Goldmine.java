...
public class Goldmine implements ResourceBuilding{
	...
	public Goldmine(OfflinePlayer owner,int level){
		level2 = level;
		owner2 = owner;
		isRealBuilding=false;
	}
	
	private Goldmine(OfflinePlayer owner,Location loc,int level,int BuildingID, boolean isreal){
	...
	public static Goldmine getGoldmine(int BuildingID,
		...
		return new Goldmine(owner,new Location(world,x,y,z),dataconf.getInt("goldmine."+BuildingID+".level"),BuildingID,true);
	}
	...
	public void setCollectable(int collectable) {
		...
		if(level2 == 0){return;}
		if(isRealBuilding() == false){return;}
		((Player) owner2).sendMessage(this+"");
		Long lastcollect;
