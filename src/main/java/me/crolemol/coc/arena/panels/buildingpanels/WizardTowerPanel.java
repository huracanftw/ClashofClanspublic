...
public class WizardTowerPanel implements BuildingPanel{
	WizardTower building;
	public WizardTowerPanel(WizardTower building){
		this.building = building;
	}
	@Override
	public Inventory getInventory(){
		BuildingSpecs[] spec = building.getBuildingSpecs();
		Inventory inv2 = Bukkit.createInventory(null, 9, "Wizard Tower");
		ItemStack Info = new ItemStack(Material.BOOK_AND_QUILL);
		ItemMeta InfoMeta = Info.getItemMeta();
		InfoMeta.setDisplayName(ChatColor.LIGHT_PURPLE+"Info");
		List<String> list = new ArrayList<String>();
		list.add("The Ultimate Arcane Defense!");
		list.add("Tower Wizards cast powerful area effect spells");
		list.add("that target both flying and ground troops.");
		...
		list2.add("Upgrade your Wizard Tower to");
		list2.add("increase its magical power!");
